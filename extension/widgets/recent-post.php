...
class clinic_recent_post_widget extends WP_Widget {
	/* Widget setup */
	...
	function form( $instance ) {
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'select_cat' ) ); ?>">
				<?php esc_attr_e( 'Select Categories:', 'clinic' ); ?>
            </label>
            ...
            <select id="<?php echo esc_attr( $this->get_field_id( 'select_cat' ) ); ?>"
                ...
                <option value="0" <?php echo( in_array( 0, $select_cat ) ? 'selected="selected"' : '' ); ?>>
					<?php esc_html_e( 'All Category', 'clinic' ); ?>
                </option>
...
// Register widget
function clinic_recent_post_widget_register() {
	register_widget( 'clinic_recent_post_widget' );
}

add_action( 'widgets_init', 'clinic_recent_post_widget_register' );